...
exports.updatePassword = async (req, res) => {
    try {
        ...
        if (!isMatch) {
            return res.status(400).json({
                success: false,
                message: "Inncorrect Old password",
            })
        }
...
exports.deleteMyProfile = async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < posts.length; i++) {
            ...
            await cloudnary.v2.uploader.destroy(post.image.public_id);
            await post.remove();
        ...
        //removing all likes of the user from all posts  const allPosts = await Post.find();
